...
{
	createBuffer(verts, numVerts, indicies, numIndicies, deviceObject);
}


Mesh::Mesh(const char* fileName, Microsoft::WRL::ComPtr<ID3D11Device> deviceObject)
{
	...
	//create the min and max bound of the bidning box
	for (int i = 0; i < positions.size(); i++)
	{
		if (positions[i].x < minBound.x)
		{
			minBound.x = positions[i].x;
		}
		if (positions[i].y < minBound.y)
		{
			minBound.y = positions[i].y;
		}
		if (positions[i].z < minBound.z)
		{
			minBound.z = positions[i].z;
		}
		if (positions[i].x > maxBound.x)
		{
			maxBound.x = positions[i].x;
		}
		if (positions[i].y > maxBound.y)
		{
			maxBound.y = positions[i].y;
		}
		if (positions[i].z > maxBound.z)
		{
			maxBound.z = positions[i].z;
		}
	}
...
DirectX::XMFLOAT3 Mesh::getMinBound()
{
	return minBound;
}

DirectX::XMFLOAT3 Mesh::getMaxBound()
{
	return maxBound;
}

void Mesh::createBuffer(Vertex* verts, int numVerts, unsigned int* indicies, int numIndicies, Microsoft::WRL::ComPtr<ID3D11Device> deviceObject)
{
